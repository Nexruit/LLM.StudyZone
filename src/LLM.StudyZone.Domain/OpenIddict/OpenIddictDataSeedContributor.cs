...
namespace LLM.StudyZone.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("StudyZone") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "StudyZone", DisplayName = "StudyZone API", Resources = { "StudyZone" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var consoleAndAngularClientId = configurationSection["StudyZone_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["StudyZone_App:RootUrl"]?.TrimEnd('/');
        ...
        //Mobile Client
        var mobileClientId = configurationSection["StudyZone_Mobile:ClientId"];
        if (!mobileClientId.IsNullOrWhiteSpace())
        {
            var mobileClientRootUrl = configurationSection["StudyZone_Mobile:RootUrl"]?.Replace("_", "-");
            await CreateApplicationAsync(
                ...
                scopes: commonScopes,
                redirectUris: new List<string> { mobileClientRootUrl },
                postLogoutRedirectUris: new List<string> { mobileClientRootUrl }
            );
        }
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["StudyZone_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["StudyZone_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        var webPublicTieredClientId = configurationSection["StudyZone_Web_Public:ClientId"];
        if (!webPublicTieredClientId.IsNullOrWhiteSpace())
        {
            var webPublicTieredRootUrl = configurationSection["StudyZone_Web_Public:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["StudyZone_Web_Public:ClientSecret"] ?? "1q2w3e*",
